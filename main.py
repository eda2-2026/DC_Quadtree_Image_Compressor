...
def comprimir(entrada, saida, limiar):
    ...
    print(
        f"    -> Concluído! PSNR: {psnr_val:.2f} dB, Folhas: {stats['total_folhas']}, Tamanho: {tamanho_bytes} bytes"
    )
...
def grafico_limiares(resultados):
    ...
    ax3.plot(
        ...
        linestyle="-",
        color=cor3,
        label="Tamanho Comprimido (Bytes)",
    )
    ax3.axhline(
        y=tamanho_original,
        color="purple",
        linestyle=":",
        label=f"Tamanho Original ({tamanho_original} bytes)",
    )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(imagem_teste):
        print(
            f"Aviso: Imagem não encontrada. Gerando imagem sintética em '{imagem_teste}'..."
        )
    ...
    res_comum = comprimir(imagem_teste, saida_teste, limiar=20.0)
    ...
    # Limpeza
    if os.path.exists(saida_teste):
        os.remove(saida_teste)